...
SKILL_NAME="xhs-parse"
WORK_DIR="$(mktemp -d /tmp/xhs-parse-skill-XXXXXX)"
trap 'rm -rf "$WORK_DIR"' EXIT
TS="$(date +%Y%m%d%H%M%S)"
RELEASE_PATH="xhs-parse/release"
...
"$ROOT_DIR/scripts/upload-file.sh" --file "$ROOT_DIR/skills/xhs-parse/scripts/run.sh" --name "${SKILL_NAME}.sh" >/dev/null
